...
p "verify====================="
    hash = params.slice(:code, :shop, :hmac, :signature, :timestamp)
    p hash

    received_signature = hash.except!(:hmac,:signature)
    p received_signature
...
p '3333333===processed request combined'
p calculated_signature
...
generated_received_signature = OpenSSL::HMAC.hexdigest(digest, secret, message)


p '44444---final'
p received_signature
p generated_received_signature
    return params[:hmac] == generated_received_signature
  end
